    ...
    if (selectedAppointment.id) {
      if (selectedAppointment.booked) {
        ...
        try {
          ...
          setEvents(
            events
              .filter((event) => event.id !== selectedAppointment.id)
              .concat([updatedEvent])
          );
          setSelectedAppointment({});
        } catch (error) {
        ...
        try {
          ...
          setEvents(
            events.filter((event) => event.id !== selectedAppointment.id)
          );
          setSelectedAppointment({});
        } catch (error) {
          console.error("Error deleting appointment:", error);
              ...
              {events.length > 0 ? (
                events.map((appointment) => (
                  ...
                  >
                    ...
                    <IconButton
                      aria-label="delete"
                      onClick={() => handleDeleteAppointment(appointment.id)} // Calls the delete handler
                    >
